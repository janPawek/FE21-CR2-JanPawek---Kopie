...
for (let i = 0; i < btn.length; i++) {
    btn[i].addEventListener('click', function() {
        if (tasks[i].importance < 5) {
            ...
            checkcolor(btn[i],tasks[i].importance)
        }
    });
    checkcolor(btn[i],tasks[i].importance)
...
function checkcolor(btn,importance){
...
// new try after deadline to sort to actual priority
document.getElementById("sort").addEventListener("click",function(){
    tasks.sort((b, a) => b.importance - a.importance);
    result.innerHTML = "";
    printtasks()
})
